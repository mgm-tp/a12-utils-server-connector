...
export class AuthorizationHeaderFilter implements RequestFilter {
	...
	canHandleRequest(_requestInit: RequestFilterPayload): boolean {
		return true;
	}
	doRequestFilter(request: RequestFilterPayload): RequestFilterResult {
		...
		const tokenFromStore = request.payload.extendedData
			? (request.payload.extendedData as TokenExtendData).token
			: "";
		const tokenTypeFromStore = request.payload.extendedData
			? (request.payload.extendedData as TokenExtendData).tokenType
			: "";
		const token = `${tokenTypeFromStore} ${tokenFromStore}`;
		if (token) {
