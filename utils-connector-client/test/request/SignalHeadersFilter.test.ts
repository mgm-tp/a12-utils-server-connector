...
import { SignalHeadersFilter } from "../../src/internal/filter/request/SignalHeadersFilter.js";

describe("com.mgmtp.a12.connector.request.signalHeaderFilter", () => {
	it("test signal headers filter", async () => {
		const controller = new AbortController();
		const requestFilterChain = new FilterChain();
		requestFilterChain.registerFilterRequest(new HeadersFilter());
		requestFilterChain.registerFilterRequest(new SignalHeadersFilter());
