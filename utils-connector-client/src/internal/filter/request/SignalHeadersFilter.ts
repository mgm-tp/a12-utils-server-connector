...
export class SignalHeadersFilter implements RequestFilter {
	canHandleRequest(requestPayload: RequestFilterPayload): boolean {
		return requestPayload.payload.signal !== undefined;
	}

	doRequestFilter(requestPayload: RequestFilterPayload): RequestFilterResult {
		requestPayload.request.signal = requestPayload.payload.signal;
		return {
